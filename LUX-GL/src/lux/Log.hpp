...
namespace lux {
	...
	public:
        ...
        static std::string format_message(const S& format_str, const Args&... args)
        {
            //std::string message = "";
            //auto size = std::sprintf(message.c_str(), format_str.c_str(), args...);
            //return message;

            //const char* fmt = "sqrt(2) = %f";

            int sz = std::snprintf(nullptr, 0, format_str, args...);
            std::vector<char> buf(sz + 1); // note +1 for null terminator
            std::snprintf(&buf[0], buf.size(), format_str, args...);
		...
		static void LOG_DEBUG(const S& format_str, const Args&... args)
		{
            ...
            const std::string message = format_message(format_str, args...);
			//fmt::printf("%02d:%02d:%02d [DEBUG] %s\n", now.Hour, now.Minute, now.Second, message);
            spdlog::debug(format_str, message);
		}
		...
		static void LOG_INFO(const S& format_str, const Args&... args)
		{
            ...
            const auto message = format_message(format_str, args...);
            ...
            spdlog::info(format_str, message);
		}
		...
		static void LOG_ERROR(const S& format_str, const Args&... args)
		{
			...
			//fmt::printf("%02d:%02d:%02d [%s] %s\n", now.Hour, now.Minute, now.Second, level, message);
            spdlog::error(format_str, message);
		}
		...
		static void LOG_FATAL(const S& format_str, const Args&... args)
		{
			...
			//fmt::printf("%02d:%02d:%02d [%s] %s\n", now.Hour, now.Minute, now.Second, level, message);
            spdlog::critical(format_str, message);
		}
