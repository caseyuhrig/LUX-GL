...
namespace lux {
    ...
    public:
        ...
        Canvas(const uint32_t width, const uint32_t height, const uint32_t samples)
            : m_Width(width), m_Height(height), m_Samples(samples), m_Multisample(samples > 1)
        {
            ...
            if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
                spdlog::error("Bloom_FRAMEBUFFER:: Framebuffer is not complete!");
            else spdlog::info("Bloom Framebuffer GOOD!");
            ...
            else spdlog::info("Framebuffer GOOD!");
            glBindFramebuffer(GL_FRAMEBUFFER, 0);
